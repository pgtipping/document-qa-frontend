...
const apiModule = jest.requireActual("@/app/api/quiz/generate/route");
...
describe("Quiz Generation API", () => {
    beforeEach(() => {
        ...
        documentProcessing.getDocumentTextContent.mockResolvedValue("This is the document content for testing with sufficient length to pass validation. This is a longer text that should be more than 100 characters to pass the validation in the quiz generation API. This text should be sufficient.");
        // Setup mock LLM responses - clear previous mock implementation first
        llmService.getCompletion.mockReset();
    ...
    it("should generate a quiz successfully", async () => {
        ...
        expect(documentProcessing.getDocumentTextContent).toHaveBeenCalledWith("document.pdf");
        // Verify title generation
        expect(llmService.getCompletion).toHaveBeenCalledWith(expect.stringContaining("Create a short but descriptive title"));
        // Verify question generation
        expect(llmService.getCompletion).toHaveBeenNthCalledWith(2, expect.stringContaining("generate 5 quiz questions"));
    ...
    it("should return 400 if document content is too short", async () => {
        ...
        documentProcessing.getDocumentTextContent.mockResolvedValue("Too short");
        // Create mock request
        const request = createMockRequest({
            documentId: "doc123",
        });
    ...
    it("should handle database errors when creating quiz", async () => {
        ...
        // Mock database error
        prisma.quiz.create.mockRejectedValue(new Error("Database connection error"));
        // Create mock request
        const request = createMockRequest({
            documentId: "doc123",
        });
    ...
    it("should use default values for missing optional parameters", async () => {
        ...
        expect(llmService.getCompletion).toHaveBeenNthCalledWith(2, expect.stringContaining("generate 5 quiz questions") // Default quiz size
        );
    });
});
