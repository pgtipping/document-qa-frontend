...
describe("Quiz Results API", () => {
  beforeEach(() => {
    ...
    (prisma.quizResult.findMany as jest.Mock).mockResolvedValue(
      mockQuizResults
    );
  ...
  describe("GET - Single Result", () => {
    it("should retrieve a specific quiz result when authenticated and authorized", async () => {
      ...
      expect(data.responses[0].questionText).toBe(
  ...
  it("should handle database errors gracefully", async () => {
    // Mock database error
    (prisma.quizResult.findUnique as jest.Mock).mockRejectedValue(
