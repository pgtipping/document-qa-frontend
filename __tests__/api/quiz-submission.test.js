...
const mockQuiz = {
    ...
    questions: [
        ...
        },
        {
            ...
            options: null,
            correctAnswer: "The water cycle describes how water evaporates from the surface of the earth, rises into the atmosphere, cools and condenses into rain or snow, and falls again to the surface.",
            explanation: "The water cycle is the continuous movement of water within the Earth and atmosphere.",
            points: 15,
        },
    ],
};
...
describe("Quiz Submission API", () => {
    beforeEach(() => {
        ...
        prisma.quizResult.create.mockResolvedValue(mockInitialQuizResult);
        // Setup mock quiz response creation
        prisma.quizResponse.create.mockImplementation(({ data }) => {
            // Find the matching mock response
            const mockResponse = mockQuizResponses.find((resp) => resp.questionId === data.questionId);
            return Promise.resolve(mockResponse);
        });
        // Setup mock quiz result update
        prisma.quizResult.update.mockResolvedValue(mockUpdatedQuizResult);
        // Setup mock LLM for short answer evaluation
        llmService.getCompletion.mockResolvedValue("CORRECT");
    });
    it("should successfully submit and grade quiz answers", async () => {
        ...
        expect(llmService.getCompletion).toHaveBeenCalledWith(expect.stringContaining("Evaluate if the user's answer is correct"));
    ...
    it("should handle LLM evaluation for short answers", async () => {
        ...
        llmService.getCompletion.mockResolvedValue("INCORRECT");
        ...
        prisma.quizResponse.create.mockResolvedValueOnce({
            id: "resp3",
            resultId: "result123",
            questionId: "q3",
        ...
        prisma.quizResult.update.mockResolvedValueOnce({
    ...
    it("should handle database errors during submission", async () => {
        ...
        prisma.quizResult.create.mockRejectedValue(new Error("Database connection error"));
        // Create mock request
        const request = createMockRequest({
            answers: [{ questionId: "q1", userAnswer: "Paris" }],
            timeTaken: 100,
