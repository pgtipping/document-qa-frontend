...
describe("QuizResults", () => {
    ...
    it("renders loading state initially", () => {
        render(_jsx(QuizResults, { quizId: mockQuizId, resultId: mockResultId }));
    ...
    it("renders quiz result data after loading", async () => {
        ...
        await waitFor(() => {
            ...
            expect(screen.getByText("From document: test-document.pdf")).toBeInTheDocument();
    ...
    it("displays feedback message", async () => {
        render(_jsx(QuizResults, { quizId: mockQuizId, resultId: mockResultId }));
        await waitFor(() => {
            expect(screen.getByText("Good job! You have a solid understanding of the material.")).toBeInTheDocument();
        });
    });
    ...
    it("handles sharing settings toggle", async () => {
        ...
        await waitFor(() => {
            expect(global.fetch).toHaveBeenCalledWith(`/api/quiz/${mockQuizId}/results`, {
                method: "POST",
                headers: {
    ...
    it("displays share URL when results are shared", async () => {
        ...
        render(_jsx(QuizResults, { quizId: mockQuizId, resultId: mockResultId }));
        await waitFor(() => {
            expect(screen.getByRole("switch", { name: "Share Results" })).toBeChecked();
            // Look for the share URL
            expect(screen.getByText(/https:\/\/example.com\/quiz\/quiz-123\/results\?resultId=result-456&shareCode=abc123xyz/)).toBeInTheDocument();
        });
    });
    it("copies share URL to clipboard when copy button is clicked", async () => {
        ...
        expect(navigator.clipboard.writeText).toHaveBeenCalledWith("https://example.com/quiz/quiz-123/results?resultId=result-456&shareCode=abc123xyz");
    });
    it("opens share dialog when share button is clicked", async () => {
        ...
        expect(screen.getByText("Your quiz results are currently private. Enable sharing to get a link.")).toBeInTheDocument();
    });
    it("navigates to create new quiz with correct document ID", async () => {
        ...
        expect(mockRouter.push).toHaveBeenCalledWith("/quiz/new?documentId=doc-123");
    });
    it("handles API errors during result fetching", async () => {
        ...
        render(_jsx(QuizResults, { quizId: mockQuizId, resultId: "invalid-result" }));
        await waitFor(() => {
            expect(screen.getByText("Error")).toBeInTheDocument();
            expect(screen.getByText("There was a problem loading the quiz results")).toBeInTheDocument();
