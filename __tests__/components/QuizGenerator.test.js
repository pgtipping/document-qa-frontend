...
describe("QuizGenerator", () => {
    ...
    it("renders the component with correct initial state", () => {
        render(_jsx(QuizGenerator, { documents: mockDocuments }));
        ...
        expect(screen.getByText("Create a quiz from your documents to test understanding")).toBeInTheDocument();
    ...
    it("shows empty state when no documents are available", () => {
        render(_jsx(QuizGenerator, { documents: [] }));
    ...
    it("allows selecting difficulty level", async () => {
        ...
        expect(screen.getByRole("combobox", { name: /difficulty/i })).toHaveTextContent("Hard");
    });
    ...
    it("handles quiz generation successfully", async () => {
        ...
        render(_jsx(QuizGenerator, { documents: mockDocuments, onGenerateStart: mockOnGenerateStart, onGenerateComplete: mockOnGenerateComplete }));
        // Select a document
        fireEvent.click(screen.getByRole("combobox", { name: /select document/i }));
        const option = await screen.findByText("Document 1.pdf");
        fireEvent.click(option);
        ...
        await waitFor(() => {
            expect(screen.getByText(/Quiz "Test Quiz on Document 1" generated successfully with 5 questions!/i)).toBeInTheDocument();
    ...
    it("handles API errors during quiz generation", async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText("Document too short for quiz generation")).toBeInTheDocument();
        });
    });
    ...
    it("resets the form when 'Reset' button is clicked", async () => {
        ...
        expect(screen.queryByRole("button", { name: "Open Quiz" })).not.toBeInTheDocument();
    });
