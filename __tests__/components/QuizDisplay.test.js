...
describe("QuizDisplay", () => {
    ...
    beforeEach(() => {
        ...
        global.fetch.mockImplementation((url) => {
            ...
            if (url === `/api/quiz/${mockQuizId}` &&
                global.fetch.mock.calls[0][1]?.method === "POST") {
                return Promise.resolve({
                    ok: true,
    ...
    it("renders loading state initially", () => {
        render(_jsx(QuizDisplay, { quizId: mockQuizId }));
    ...
    it("renders short answer questions correctly", async () => {
        ...
        expect(screen.getByText("Explain the water cycle in your own words.")).toBeInTheDocument();
    ...
    it("allows typing in short answer questions", async () => {
        ...
        expect(textArea).toHaveValue("Water evaporates, forms clouds, and then precipitates.");
    });
    it("submits the quiz on final question", async () => {
        const mockOnComplete = jest.fn();
        render(_jsx(QuizDisplay, { quizId: mockQuizId, onComplete: mockOnComplete }));
        await waitFor(() => {
            expect(screen.getByText("What is the capital of France?")).toBeInTheDocument();
        });
    ...
    it("shows error state when quiz fails to load", async () => {
        ...
        render(_jsx(QuizDisplay, { quizId: "invalid-quiz" }));
        await waitFor(() => {
            expect(screen.getByText("Error")).toBeInTheDocument();
            expect(screen.getByText("There was a problem loading the quiz")).toBeInTheDocument();
    ...
    it("calls router.push when no onComplete is provided", async () => {
        ...
        await waitFor(() => {
            expect(mockRouter.push).toHaveBeenCalledWith("/quiz/quiz-123/results?resultId=result-123");
        });
    });
});
