...
export async function GET(request, { params }) {
    try {
        ...
        if (quiz.userId !== userId && !quiz.isPublic) {
            return NextResponse.json({ error: "You don't have access to this quiz" }, { status: 403 });
        }
...
export async function POST(request, { params }) {
    try {
        ...
        if (!answers || !Array.isArray(answers)) {
            return NextResponse.json({ error: "Invalid answers format. Expected array of answers." }, { status: 400 });
        }
        ...
        const questionMap = new Map(quiz.questions.map((q) => [
            q.id,
            {
                id: q.id,
