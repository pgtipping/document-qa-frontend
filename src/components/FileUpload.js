...
const IconWrapper = ({ Icon, size, className, }) => _jsx(Icon, { size: size, className: className });
export default function FileUpload() {
    ...
    const handleUpload = useCallback(async (acceptedFiles) => {
        if (acceptedFiles.length === 0 || !isAuthenticated)
    ...
    return (_jsxs("div", { className: "w-full space-y-4", children: [_jsxs("div", { ...getRootProps(), className: `
        ...
        `, children: [_jsx("input", { ...getInputProps() }), isLoadingSession ? (_jsx("div", { className: "text-center text-muted-foreground", children: "Loading session..." })) : !isAuthenticated ? (_jsxs("div", { className: "text-center text-muted-foreground", children: ["Please", " ", _jsx(Link, { href: "/api/auth/signin", className: "underline text-primary", children: "log in" }), " ", "to upload documents."] })) : (_jsxs("div", { className: "flex flex-col items-center justify-center space-y-4 text-center", children: [_jsx("div", { className: `
              p-4 rounded-full 
              transition-all duration-200
              ${isDragActive
                                    ? "bg-primary/10 text-primary"
                                    : "bg-muted text-muted-foreground"}
            `, children: _jsx(IconWrapper, { Icon: Upload, size: 32 }) }), _jsxs("div", { className: "space-y-2", children: [_jsx("p", { className: "text-base font-medium", children: isDragActive ? (_jsx("span", { className: "text-primary", children: "Drop your file here" })) : ("Drag & drop your file here") }), _jsx("p", { className: "text-sm text-muted-foreground", children: "or click to browse from your computer" })] }), _jsx("div", { className: "text-xs text-muted-foreground/75", children: "Supports PDF, TXT, DOC, and DOCX files up to 10MB" })] }))] }), file &&
                isAuthenticated && ( // Only show file details if authenticated
            _jsxs("div", { className: "flex flex-col space-y-2", children: [_jsxs("div", { className: "flex items-center justify-between p-4 border rounded-lg bg-muted/50", children: [_jsxs("div", { className: "flex items-center space-x-4", children: [_jsx("div", { className: "p-2 rounded-full bg-primary/10", children: _jsx(IconWrapper, { Icon: File, size: 24, className: "text-primary" }) }), _jsxs("div", { children: [_jsx("p", { className: "text-sm font-medium truncate max-w-[200px]", children: file.name }), _jsxs("p", { className: "text-xs text-muted-foreground", children: [(file.size / 1024 / 1024).toFixed(2), " MB"] })] })] }), _jsx(Button, { variant: "ghost", size: "sm", className: "text-muted-foreground hover:text-destructive", onClick: removeFile, disabled: isUploading, children: _jsx(IconWrapper, { Icon: X, size: 16 }) })] }), isUploading && (_jsxs("div", { className: "space-y-2", children: [_jsx(Progress, { value: uploadProgress, className: "w-full" }), _jsxs("p", { className: "text-xs text-center text-muted-foreground", children: ["Uploading... ", Math.round(uploadProgress), "%"] })] }))] }))] }));
}
