        ...
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                try {
                    ...
                    if (!user) {
                        console.log(`Credentials provider: No user found for email: ${credentials.email}`);
                        return null; // User not found
                    }
                    // 2. Check if user has a password set (they might have signed up via OAuth)
                    if (!user.password) {
                        console.log(`Credentials provider: User ${credentials.email} has no password set (likely OAuth user).`);
                    ...
                    const passwordsMatch = await bcrypt.compare(credentials.password, user.password // Assuming user.password stores the hashed password
                    );
                    if (!passwordsMatch) {
                        console.log(`Credentials provider: Invalid password for user: ${credentials.email}`);
                        return null; // Passwords don't match
                    }
                    console.log(`Credentials provider: User ${credentials.email} authenticated successfully.`);
                    ...
                    console.error("Credentials provider: Error during authorization:", error);
...
export async function withAdminAuth(req, handler) {
    ...
    if (!process.env.ADMIN_EMAIL) {
        ...
        return new NextResponse("Internal Server Error: Server configuration issue", { status: 500 });
    }
    if (session.user.email !== process.env.ADMIN_EMAIL) {
        console.warn(`Admin access denied for user: ${session.user.email}. Required: ${process.env.ADMIN_EMAIL}`);
