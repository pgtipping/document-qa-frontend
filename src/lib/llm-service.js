...
if (openaiApiKey) {
    ...
    console.warn("OpenAI API key not found (OPENAI_API_KEY). Embedding generation will fail.");
}
...
const fallbackProviders = [
    {
        ...
        client: null, // Client will be initialized if API key exists
        init: function () {
            if (this.apiKey) {
    ...
    },
    {
        ...
        client: null,
        genModel: null, // Use the specific type
        init: function () {
            if (this.apiKey) {
...
const availableProviders = fallbackProviders.filter((provider) => provider.init());
if (availableProviders.length === 0) {
    console.error("FATAL: No LLM providers could be initialized. Check API keys.");
    // Depending on application requirements, you might throw an error here
    // or allow the app to run in a degraded state.
}
else {
    console.log(`Initialized LLM providers: ${availableProviders
        .map((p) => p.name)
        .join(", ")}`);
}
// --- Core Function ---
...
export async function getCompletion(prompt) {
    ...
    for (const provider of availableProviders) {
        console.log(`Attempting completion with ${provider.name} (${provider.model})...`);
        try {
            const startTime = Date.now();
            const result = await provider.call(prompt);
            const duration = Date.now() - startTime;
            if (result) {
                console.log(`Completion successful with ${provider.name} in ${duration}ms.`);
                // TODO: Add performance logging here if needed
                return result;
            }
            else {
                console.warn(`Provider ${provider.name} returned null or empty result.`);
            }
        }
        catch (error) {
            console.error(`Error calling ${provider.name}:`, error instanceof Error ? error.message : error);
 ...
 */
export async function getExtractionFallback(prompt) {
    ...
    console.log(`Attempting extraction fallback with ${googleProvider.name} (${googleProvider.model})...`);
    try {
        const startTime = Date.now();
        ...
        const duration = Date.now() - startTime;
        if (result) {
            console.log(`Extraction fallback successful with ${googleProvider.name} in ${duration}ms.`);
            return result;
        }
        else {
            console.warn(`Extraction fallback provider ${googleProvider.name} returned null or empty result.`);
            return null;
        }
    }
    catch (error) {
        console.error(`Error calling ${googleProvider.name} for extraction fallback:`, error instanceof Error ? error.message : error);
        return null;
    }
}
/**
...
export async function generateEmbedding(text) {
    ...
    try {
        console.log(`Generating embedding for text snippet (length: ${inputText.length})...`);
        ...
        if (embeddingResponse &&
            ...
            console.error("Invalid response structure received from OpenAI embedding API:", embeddingResponse);
            return null;
        }
    }
    catch (error) {
 ...
 */
export function splitIntoChunks(text, maxChunkSizeChars = 1500, // Target size in characters
overlapChars = 200 // Overlap characters between chunks
) {
    ...
    console.log(`Splitting text (${text.length} chars) into chunks (max: ${maxChunkSizeChars}, overlap: ${overlapChars})...`);
    const chunks = [];
    let startIndex = 0;
 ...
 */
export function getRelevantChunks(chunks, question, maxChunks = 5) {
    console.log(`Selecting relevant chunks using basic keyword matching for question: "${question}"`);
    ...
    const selectedChunks = scoredChunks
        .slice(0, maxChunks)
        .map((item) => item.chunk);
    console.log(`Selected ${selectedChunks.length} chunks based on keyword relevance.`);
 ...
 */
export function createPromptTemplate(contextChunks, question) {
...
export function countTokens(text) {
    if (encoding) {
        try {
            ...
            console.warn("Tiktoken encoding failed during count, falling back:", error);
 ...
 */
export function buildPromptWithContextLimit(relevantChunks, question, basePromptTemplate = `Based on the following context, please answer the question. If the answer is not present in the context, say "I cannot find the answer in the provided documents."\n\nContext:\n{context}\n\nQuestion: {question}\n\nAnswer:`, maxTokens = MAX_CONTEXT_TOKENS) {
    const questionTokens = countTokens(question);
    // Estimate tokens for the template structure itself (excluding placeholders)
    const templateTokens = countTokens(basePromptTemplate.replace("{context}", "").replace("{question}", ""));
    ...
    if (availableTokensForContext <= 0) {
        console.warn(`Not enough tokens for context (${availableTokensForContext}) after accounting for question (${questionTokens}) and template (${templateTokens}). Max: ${maxTokens}`);
    ...
    console.log(`Building prompt. Max context tokens: ${availableTokensForContext}`);
    for (const chunk of relevantChunks) {
        const chunkTokens = countTokens(chunk);
        const tokensWithSeparator = chunkTokens + (includedChunks.length > 0 ? joinerTokens : 0);
        if (currentTokens + tokensWithSeparator <= availableTokensForContext) {
            currentTokens += tokensWithSeparator;
            includedChunks.push(chunk);
        }
        else {
            console.log(`Token limit (${availableTokensForContext}) reached while adding chunks. Used ${currentTokens} tokens. Stopping context inclusion.`);
            break; // Stop adding chunks once limit is exceeded
        }
    }
    const finalContext = includedChunks.length > 0
        ? includedChunks.join("\n\n---\n\n")
    ...
    console.log(`Included ${includedChunks.length}/${relevantChunks.length} relevant chunks. Final prompt tokens: ~${finalTokenCount} (Context tokens: ~${currentTokens})`);
    if (finalTokenCount > maxTokens) {
        console.warn(`Final prompt token count (${finalTokenCount}) still exceeds maxTokens (${maxTokens}) despite limiting. This might indicate issues with token estimation or template size.`);
